...
**Your Answers here:**
```
1. Line 8: mytickers = c("SPY", "AAPL"). Tickers defined as Apple and SPY
("helpersSet1.R") uses this .R file

2. cumulative: 577.2
average daily returns: 0.149
sd: 2.246
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
transposedData = t(StockReturns)
dual = apply(transposedData,2,mean)
names(dual) <- colnames(transposedData)
pnl_plot(dual)
```
...
Using companies: Johnson and Johnson, Sanofi, Glaxo-Smtih Kline and SPY to form a healthcare portfolio

source("helpersSet1.R")
source("dataSet2.R")

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
Healthportfolio = apply(StockReturns,1,mean)
names(Healthportfolio) <- rownames(StockReturns)
pnl_plot(Healthportfolio)
```
...
**Your Answers here:**
```
1. you get the sum of the number of rows i.e. 20 rows of stock returns.

2. this joins "mydata" with stock returns from the first 40 days
therefore you should get "40"
...
```
Loaded data set Judy.csv
Mean office SBP- 169.6mmHg
sd office SBP- 24.2 mmHg
```
